...
echo "$FAKE_MODEL" >> .subagent/models_seen.txt
if printf '%s' "$FAKE_PROMPT" | grep -q "You are a code reviewer"; then
  n=$(( $(cat .subagent/review_count 2>/dev/null || echo 0) + 1 )); echo $n > .subagent/review_count
  if [ "$AUTO_SCENARIO" = concerns ] || { [ "$AUTO_SCENARIO" = fixloop ] && [ $n = 1 ]; }; then
    echo '{"verdict":"concerns","issues":[{"severity":"high","file":"src/add.js","line":1,"issue":"uses eval"}]}' > .subagent/review/result.json
  else
    echo '{"verdict":"ok","issues":[]}' > .subagent/review/result.json
  fi
  exit 0
fi
case "$AUTO_SCENARIO" in
  dispute)
    echo "test 'adds' expects 5 but spec says 6" > .subagent/test_change_request.md
    echo '{"status":"needs_test_change","summary":"disputed"}' > .subagent/result.json; exit 0 ;;
  escalate)
...
echo '{"status":"done","summary":"worker finished"}' > .subagent/result.json

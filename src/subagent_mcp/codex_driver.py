...
Parallel to the claude driver in runs.py. `CodexAgent` reuses the Agent run
loop (queue, verification, distillation, trips, trace) and replaces only the
spawn: argv, environment and the event stream. Codex's JSONL events are
...
class CodexAgent(Agent):
    """An Agent whose turns run on ``codex exec``. session_id is Codex's thread_id."""

    needs_api_key = False  # the Codex CLI logs in with its own auth.json

    @property
    def guard(self) -> str:  # type: ignore[override]
        return GUARD_NO_HOOK if self.settings.supervisor == "off" else GUARD

    def _boot(self) -> None:
        try:
            self.codex_home = prepare_home(self.settings, self.agent_id, self.lane)
            binary = codex_bin()
            if shutil.which(binary) is None:
                self._start_error = f"{binary!r} is not on PATH; install the Codex CLI"
            elif not (source_codex_home() / "auth.json").exists():
                self._start_error = (
                    f"no Codex login at {source_codex_home() / 'auth.json'}; run `codex login`"
                )
    ...
    def _spawn(self, prompt: str, resume: str | None) -> CodexProcess:  # type: ignore[override]
        home = getattr(self, "codex_home", None) or prepare_home(
            self.settings, self.agent_id, self.lane
        )
        argv = codex_argv(self.workspace, self.model or None, resume)
        env = codex_env(self.settings, self.agent_id, home)
        return _spawn_codex(argv, env, str(self.workspace), prompt)

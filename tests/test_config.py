...
from glm_subagent_mcp.config import DEFAULT_BASE_URL, Settings
...
def test_child_env_strips_parent_oauth_and_sets_glm(tmp_path: Path, monkeypatch):
    ...
    settings = make_settings(tmp_path, api_key="glm-key")
...
def test_from_env_reads_gsa_workspace(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("GSA_WORKSPACE", str(tmp_path))
    monkeypatch.setenv("GSA_MODEL", "glm-5.3-flash[1m]")
    monkeypatch.setenv("GLM_API_KEY", "from-env")
    settings = Settings.from_env()
    assert settings.workspace == tmp_path.resolve()
    assert settings.model == "glm-5.3-flash[1m]"
    assert settings.api_key == "from-env"


def test_compact_window_from_env(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("GSA_WORKSPACE", str(tmp_path))
    monkeypatch.setenv("GSA_COMPACT_WINDOW", "40960")
...
def test_loop_strikes_default_is_eight(monkeypatch):
    ...
    monkeypatch.delenv("GSA_LOOP_STRIKES", raising=False)

...
class CodeReview(Sandbox):
    def test_review_sees_code_not_tests_and_cannot_edit(self):
        ...
        log = "".join(p.read_text() for p in (root / ".subagent/logs").glob("review-*.log"))
        self.assertIn("diff has src/add.js", log)
        self.assertNotIn("LEAK", log)

    def test_verdict_given_in_chat_is_recovered(self):
        root = self.node_project()
        code, r = self.delegate("run", "--plan", ".subagent/PLAN.md", "--auto", cwd=root, scenario="revchat",
                                env={"REVIEWER": "chat"})
        self.assertEqual(r["review"]["verdict"], "concerns")
        self.assertEqual(self.read(root, ".subagent/review_attempts").strip(), "1")
    ...
    def test_no_verdict_is_retried_then_unverified(self):
        ...
        self.assertEqual(self.read(root, ".subagent/review_attempts").strip(), "2")
...
class TestReview(Sandbox):
    ...
    def run_auto(self, root, **env):
        return self.delegate("run", "--plan", ".subagent/PLAN.md", "--auto", cwd=root, scenario="testrev",
...
for_drivers(CodeReview)
for_drivers(TestReview)

...
from helpers import REPO, Sandbox, need
...
class Shells(Sandbox):
    ...
    def test_commands_in_each_shell(self):
        ...
        self.env["FAKE_SCRIPT"] = str(REPO / "tests" / "scenarios" / "capture.sh")
        for shell in shells:
            with self.subTest(shell):
                ...
                steps = [
                    ("delegate detect", lambda r: r["test_cmd"] == "npm test"),
                    ("python3 ~/.claude/skills/delegate/delegate.py detect", lambda r: "protected_files" in r),
                    ("delegate run --plan .delegate/PLAN.md --background", lambda r: r["status"] == "started"),
                    ("delegate wait --timeout 60", lambda r: r["status"] == "done"),
                    ("env DELEGATE_LIVE_VIEW=off delegate test", lambda r: r["passed"] is True),
                    ("delegate checkpoints", lambda r: len(r["checkpoints"]) >= 1),
                    ("delegate undo", lambda r: r["status"] == "undone"),
                ]
                ...
                for command in ("delegate run --plan .delegate/PLAN.md --feedback-file .delegate/feedback.md",
                                "cat .delegate/feedback.md | delegate run --plan .delegate/PLAN.md --feedback-file -"):
